...
import { formatDateBs, pluralBs } from "@/lib/utils"
...
export function Hero({
  ...
  return (
    <section className="relative overflow-hidden bg-forest-950 text-paper">
      <GeoPattern
        id="hero-pat"
        className="absolute inset-0 text-gold-300 opacity-[0.05]"
      />
      <div
        aria-hidden
        ...
        style={{
          background:
            "radial-gradient(circle, rgba(42,138,103,0.35) 0%, rgba(42,138,103,0) 70%)",
        }}
      />
      <div
        aria-hidden
        ...
        style={{
          background:
            "radial-gradient(circle, rgba(14,69,52,0.7) 0%, rgba(14,69,52,0) 70%)",
        }}
      />
      ...
      <div className="container-site relative grid items-center gap-14 py-16 md:py-20 lg:grid-cols-[1.05fr_0.95fr] lg:py-24">
          ...
          <dl className="mt-14 flex flex-wrap gap-x-12 gap-y-5 border-t border-paper/10 pt-8">
            {statItems.map((s) => (
              <div key={s.label}>
                ...
                <dd className="font-display text-3xl font-medium text-gold-300">
                  {s.n}
                </dd>
        ...
        <div className="relative hidden lg:block">
          <ArchFrame
            id="hero-arch"
            className="mx-auto w-full max-w-[400px] drop-shadow-2xl"
          />
          ...
          <div className="absolute -left-4 bottom-10 w-[300px] rounded-2xl bg-paper-card p-5 text-ink shadow-lift ring-1 ring-line xl:-left-8">
            <p className="kicker text-[0.62rem] text-gold-600">
